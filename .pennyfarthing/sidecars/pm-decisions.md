...
- **Three-layer content inheritance: base → genre → world.** Structural invariant for all content grooming. Base = archetype structure (no flavor). Genre = tone (speech, equipment, naming, visual cues). World = specific lore (faction membership, named relationships). Resolution is prototype-chain / Python-MRO style. Reject stories that propose flattening these layers or redefining archetypes per-world.

### Cliche / content quality gate
- **Cliche granularity is the quality dial.** Claude averages its training data, and the average is a cliche. The strategy is not to eliminate tropes but to pick a granularity finer than the audience's expertise wall. For Keith's high-expertise domains (software, game mechanics, RPG design, art, narrative, React, Rust-learning), cliche must drop to competent-practitioner minimum; niche-specialist via reference stacking is safer.
- **Coarse content in a high-expertise domain is the failure mode.** "Voodoo" is cliche; "Candomblé Ketu terreiro in Salvador" is authenticity. "Hacker" is cliche; "container escape via a runc CVE chain + LLM weight poisoning + RAG corpus corruption" triangulates into a niche even current red-teamers probably aren't tracking. Reject ACs that accept coarse framings when specific particulars were possible.
- **Reference stacking is the accelerator.** Three specific references triangulated into a niche drop granularity three levels in one move. Syncretism (real traditions named, seams where they collide marked) is the same technique for cultural content — a PM gate against pastiche.
